...
class InMemoryLocalDataSource : LocalDataSource {
    ...
    override suspend fun insertFarm(farm: Farm) {
        farms.value = farms.value.filter { it.id != farm.id } + farm
    }
    ...
    override suspend fun insertCropProfile(profile: CropProfile) {
        cropProfiles.value = cropProfiles.value.filter { it.id != profile.id } + profile
    }
    ...
    override suspend fun insertNode(node: Node) {
        nodes.value = nodes.value.filter { it.id != node.id } + node
    }
    ...
    override suspend fun insertSensorReading(reading: SensorReading) {
        readings.value = readings.value + reading
    }
    ...
    override suspend fun insertManualPh(entry: ManualPH) {
        manualPhs.value = manualPhs.value.filter { it.id != entry.id } + entry
    }
    ...
    override suspend fun insertSoilAnalysis(analysis: SoilAnalysis) {
        soilAnalyses.value = soilAnalyses.value + analysis
    }
    ...
    override suspend fun insertNpkResult(result: NpkResult) {
        npkResults.value = npkResults.value + result
    }
    ...
    override suspend fun insertIrrigationState(state: IrrigationState) {
        irrigationStates.value = irrigationStates.value + state
    }
    ...
    override suspend fun insertIrrigationCommand(command: IrrigationCommand) {
        irrigationCommands.value = irrigationCommands.value.filter { it.id != command.id } + command
    }
    ...
    override suspend fun insertSafetyState(state: SafetyState) {
        safetyStates.value = safetyStates.value + state
    }

    override fun observeAllAlerts(): Flow<List<Alert>> = alerts

    override suspend fun insertAlert(alert: Alert) {
        alerts.value = alerts.value.filter { it.id != alert.id } + alert
    }
    ...
    override suspend fun insertSyncRecord(record: SyncRecord) {
        syncRecords.value = syncRecords.value.filter { it.id != record.id } + record
    }
